...
import br.com.treinaweb.modelos.Tarefa;
import br.com.treinaweb.modelos.Usuario;
import br.com.treinaweb.repositorio.RepositorioTarefa;
...
public class TarefasController {
	...
	public ModelAndView inserir(@Valid Tarefa tarefa, BindingResult result, HttpServletRequest request) 
	{
		...
		if(tarefa.getDataExpiracao() == null) {
			result.rejectValue("dataExpiracao", "tarefa.dataExpiracaoInvalida", "A data "
					+ "de expiração não foi inserida");
		}else {
			if(tarefa.getDataExpiracao().before(new Date())) {
			result.rejectValue("dataExpiracao", "tarefa.dataExpiracaoInvalida", "A data "
					+ "de expiração não pode ser anteriar a data atual");
			}
		}
		if (result.hasErrors()) {
			mv.setViewName("tarefas/inserir");
